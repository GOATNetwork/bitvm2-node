...
if [ $start -ne $EL_START_BLOCK_NUMBER ]; then
  ...
  RUST_LOG=info $CMD -- --start $EL_START_BLOCK_NUMBER --batch-size $batch --init-input --output-proof "${DATA}/${start}-${batch}.bin" --blocks $DATA/blocks.bin --force-fetch
