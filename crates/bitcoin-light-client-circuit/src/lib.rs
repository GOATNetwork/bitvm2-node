...
// Regenerate these IDs after changing a leaf guest or the verifier.
// TODO: Generated by `proof-builder-rpc`
pub const EXPECTED_HEADER_CHAIN_PROGRAM_ID: verifier::ProgramId = [
    0x87, 0x6b, 0xd8, 0x4c, 0xc8, 0x9b, 0xa4, 0x57, 0xa0, 0xfb, 0x94, 0xfc, 0x20, 0x13, 0xbd, 0x75,
    0x66, 0xae, 0xcc, 0x1e, 0x04, 0xbe, 0xfa, 0x53, 0x6c, 0x0c, 0x38, 0xd9, 0x57, 0x48, 0xf4, 0xf4,
];
pub const EXPECTED_STATE_CHAIN_PROGRAM_ID: verifier::ProgramId = [
    0x73, 0xc0, 0x74, 0x70, 0xf9, 0x7b, 0x3f, 0x8d, 0xe7, 0x48, 0x94, 0xad, 0x1a, 0xec, 0x60, 0xfb,
...
pub const EXPECTED_COMMIT_CHAIN_PROGRAM_ID: verifier::ProgramId = [
    0x93, 0x91, 0xe0, 0x33, 0x40, 0x2b, 0x86, 0x2b, 0x5e, 0x74, 0xf7, 0xd6, 0x65, 0xac, 0x91, 0xcf,
    0x80, 0xc2, 0xb2, 0x6f, 0xb1, 0x0f, 0x0c, 0x72, 0xcb, 0x92, 0xc9, 0x64, 0x17, 0xc1, 0x12, 0xf6,
];
...
fn checked_history_root(
    ...
    expected_program_id: verifier::ProgramId,
    history: [u8; 32],
) -> [u8; 32] {
    assert_eq!(actual_program_id, expected_program_id, "unexpected proof program id");
...
pub fn watch_longest_chain(
    ...
    let program_history_root = verifier::program_history_root(
        checked_history_root(
            ...
            header_program_id,
            btc_header_chain_output.self_program_id,
            EXPECTED_HEADER_CHAIN_PROGRAM_ID,
            btc_header_chain_output.program_history_hash,
        ),
        checked_history_root(
            verifier::ProgramType::State,
            state_program_id,
            state_chain_output.self_program_id,
            EXPECTED_STATE_CHAIN_PROGRAM_ID,
            state_chain_output.program_history_hash,
        ),
        checked_history_root(
            verifier::ProgramType::Commit,
            commit_program_id,
            commit_chain_output.self_program_id,
            EXPECTED_COMMIT_CHAIN_PROGRAM_ID,
...
pub fn propose_longest_chain(
    ...
    graph_watchtower_xonly_public_keys: &[[u8; 32]],
    expected_watchtower_program_id: verifier::ProgramId,
    ...
    for (challenge_position, node_index) in watchtower_challenge_indices.iter().enumerate() {
        ...
        if included_watchtowers_bits[i] {
            ...
            match verify_groth16_proof(&proof, &public_values, &vk, &zkm_version) {
                Ok(program_id) if program_id == expected_watchtower_program_id => {}
                Ok(_) => {
                    println!("Watchtower[{i}] unexpected program id");
                    continue;
                }
                Err(err) => {
