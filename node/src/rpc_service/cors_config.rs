...
impl CorsConfig {
    ...
    pub fn create_cors_layer(&self) -> CorsLayer {
        ...
        if self.allowed_origins.is_empty() {
            ...
            let origins: Result<Vec<_>, _> = self.allowed_origins
                .iter()
        ...
        if self.allowed_headers.is_empty() {
            ...
            let headers: Result<Vec<_>, _> = self.allowed_headers
                .iter()
    ...
    pub fn validate_security(&self) -> Vec<String> {
        ...
        if self.allow_credentials {
            ...
            if self.allowed_origins.contains(&"*".to_string()) {
                warnings.push("CORS allows credentials with wildcard origin, security risk".to_string());
            }
            
            // Check for empty origins (falls back to wildcard)
            if self.allowed_origins.is_empty() {
                warnings.push("CORS allows credentials with empty origins list, will fallback to wildcard".to_string());
            }
            
            // Check for HTTP origins with credentials
            for origin in &self.allowed_origins {
                if origin.starts_with("http://") && !origin.contains("localhost") {
                    warnings.push(format!("CORS allows credentials with insecure HTTP origin: {}", origin));
                }
            }
            ...
            for origin in &self.allowed_origins {
                if origin.contains("*") && !origin.starts_with("https://") {
                    warnings.push(format!("CORS allows credentials with potentially unsafe origin pattern: {}", origin));
                }
            }
        }
        ...
        if self.max_age > 86400 { // More than 24 hours
            warnings.push(format!("CORS max_age is too long ({} seconds), consider reducing for better security", self.max_age));
        } else if self.max_age > 3600 { // More than 1 hour
            warnings.push(format!("CORS max_age is quite long ({} seconds), ensure this is intentional", self.max_age));
        }
