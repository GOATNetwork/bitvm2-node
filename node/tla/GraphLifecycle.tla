...
\* NOTE this differs from the README and from the original draft of this
\* spec: Obsoleted is NOT terminal in the real code. utils.rs:1418-1434
\* re-checks `matches!(current_status, PreKickoff | Obsoleted)` and can
\* move an Obsoleted graph to OperatorKickOff or Skipped if a kickoff tx
\* is later observed on-chain.
TerminalStatuses == {"OperatorTake1", "OperatorTake2", "Skipped", "Disprove"}

\* The real edge set from scan_graph_chain_state (utils.rs:1328-1681), used
\* by the CORE-ONLY config (GraphLifecycleCoreOnly.cfg) to check the chain-
\* scan state machine is internally sound in isolation.
AllowedTransitions == {
...
TypeOK == status \in GraphStatuses
